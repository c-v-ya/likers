...
from backend.services import EmailHunter
from backend.tasks import enrich
...
class SignUpRequestSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data: dict):
        ...
        if User.objects.filter(
            Q(email=email) | Q(username=validated_data.get("username"))
        ).exists():
            raise serializers.ValidationError(
                self.custom_error_messages.get("user_exists_error"), code="invalid"
            )
    ...
    def check_email(self, email):
        if not EmailHunter.email_valid(email):
            raise serializers.ValidationError(
                self.custom_error_messages.get("invalid_email"), code="invalid"
            )
